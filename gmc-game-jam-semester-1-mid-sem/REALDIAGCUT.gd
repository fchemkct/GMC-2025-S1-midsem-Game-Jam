...
var dlines = null
var waiting_for_cutscene_input = false
...
func display_next_line():
	if (current_line < dlines.lines.size()):
		if (!dlines.lines[current_line].contains("#")):
			...
			_dialogueEnd()
		elif dlines.lines[current_line].contains("#cutscence"):
			var colon_index = dlines.lines[current_line].find(":")
			var image_path = dlines.lines[current_line].substr(colon_index + 1).strip_edges()
			_show_cutscene_image(image_path)
...
func _show_cutscene_image(path: String):
